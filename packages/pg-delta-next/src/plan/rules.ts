...
function defaultPrivConsumes(id: {
  ...
  if (id.grantee !== "PUBLIC") consumes.push({ kind: "role", name: id.grantee });
...
function publicationObjects(fact: Fact): {
  ...
  const tables = (p(fact, "tables") as unknown as PublicationTableEntry[]) ?? [];
...
export const RULES: Record<string, KindRules> = {
  ...
  table: {
    ...
    create: (fact, view) => {
      ...
      const specs: ActionSpec[] = [
        { sql: createSql, consumes, alsoProduces },
      ];
  ...
  type: {
    ...
    attributes: {
      ...
      values: {
        alter: (fact, from, to) => {
          // enum growth: old values must remain a subsequence of new values;
          // each missing value becomes ALTER TYPE … ADD VALUE BEFORE/AFTER
          const id = fact.id as { schema: string; name: string };
          const relName = rel(id.schema, id.name);
          ...
          if (oldIdx !== oldValues.length) {
            throw new Error(
              `enum ${relName}: removing or reordering values requires drop+create, which existing dependents make unsafe — not supported`,
            );
          }
          return specs;
        },
  ...
  publication: {
    ...
    create: (fact) => {
      ...
      if (p(fact, "viaRoot")) withParts.push(`publish_via_partition_root = true`);
    ...
    attributes: {
      owner: ownerRule(
        (fact) => `ALTER PUBLICATION ${qid((fact.id as { name: string }).name)}`,
      ),
      publish: {
        alter: (fact, _from, to) => ({
