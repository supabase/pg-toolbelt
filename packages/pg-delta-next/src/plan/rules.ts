...
export const RULES: Record<string, KindRules> = {
  role: {
    ...
    drop: (fact) => ({
      sql: `DROP ROLE ${qid((fact.id as { name: string }).name)}`,
    }),
  ...
  table: {
    ...
    create: (fact) => {
      ...
      if (p(fact, "rowSecurity")) {
        specs.push({
          sql: `ALTER TABLE ${rel(id.schema, id.name)} ENABLE ROW LEVEL SECURITY`,
        });
      }
      if (p(fact, "forceRowSecurity")) {
        specs.push({
          sql: `ALTER TABLE ${rel(id.schema, id.name)} FORCE ROW LEVEL SECURITY`,
        });
      }
      if (str(p(fact, "owner")) !== "") {
        specs.push({
    ...
    attributes: {
      ...
      identity: {
        alter: (fact, from, to) => {
          ...
          const phrase =
            to === "a" ? "GENERATED ALWAYS" : "GENERATED BY DEFAULT";
          if (from == null)
            return { sql: `${target} ADD ${phrase} AS IDENTITY` };
  ...
  constraint: {
    ...
    drop: (fact) => {
      const id = fact.id as { schema: string; table: string; name: string };
      return {
        sql: `ALTER TABLE ${rel(id.schema, id.table)} DROP CONSTRAINT ${qid(id.name)}`,
      };
    },
    attributes: {
      def: "replace",
