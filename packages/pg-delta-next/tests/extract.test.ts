...
import { deserializeSnapshot, serializeSnapshot } from "../src/core/snapshot.ts";
...
describe("extract: fixture ring", () => {
  ...
  test("schema, table, and column facts exist with normalized payloads", () => {
    expect(fb().get({ kind: "schema", name: "app" })?.payload["owner"]).toBe("test");
    const table = fb().get({ kind: "table", schema: "app", name: "users" });
    expect(table?.payload).toMatchObject({ persistence: "p", rowSecurity: true });
    const email = fb().get({ kind: "column", schema: "app", table: "users", name: "email" });
    expect(email?.payload).toMatchObject({ type: "text", notNull: true, identity: null });
    const id = fb().get({ kind: "column", schema: "app", table: "users", name: "id" });
    expect(id?.payload).toMatchObject({ type: "integer", identity: "a" });
    const score = fb().get({ kind: "column", schema: "app", table: "users", name: "score" });
    expect(score?.payload).toMatchObject({ type: "numeric(10,2)" });
  });

  test("defaults are their own facts (pg_attrdef model)", () => {
    const def = fb().get({ kind: "default", schema: "app", table: "users", name: "score" });
    expect(def?.payload["expr"]).toBe("0.0");
    const orderDefault = fb().get({ kind: "default", schema: "app", table: "orders", name: "id" });
    expect(String(orderDefault?.payload["expr"])).toContain("nextval");
  });

  test("constraints carry canonical pg_get_constraintdef", () => {
    const pk = fb().get({ kind: "constraint", schema: "app", table: "users", name: "users_pkey" });
    expect(pk?.payload["def"]).toBe("PRIMARY KEY (id)");
    const fk = fb().get({ kind: "constraint", schema: "app", table: "orders", name: "orders_user_fk" });
    expect(fk?.payload["def"]).toBe("FOREIGN KEY (user_id) REFERENCES app.users(id)");
    expect(fk?.payload["type"]).toBe("f");
  });

  test("non-constraint index extracted with canonical def; pkey index is not", () => {
    const idx = fb().get({ kind: "index", schema: "app", name: "orders_user_idx" });
    expect(String(idx?.payload["def"])).toContain("CREATE INDEX orders_user_idx");
    expect(fb().has({ kind: "index", schema: "app", name: "users_pkey" })).toBe(false);
  });

  test("identity-column backing sequence is excluded; user sequence is present", () => {
    const seq = fb().get({ kind: "sequence", schema: "app", name: "order_seq" });
    expect(seq?.payload).toMatchObject({ start: "100", increment: "5" });
    const internal = fb()
      .facts()
      .filter((f) => f.id.kind === "sequence" && encodeId(f.id).includes("users_id"));
    expect(internal).toHaveLength(0);
  });

  test("view, function, policy, trigger-less fixture facts", () => {
    const view = fb().get({ kind: "view", schema: "app", name: "user_emails" });
    expect(String(view?.payload["def"])).toContain("FROM app.users");
    const fn = fb().get({
      kind: "procedure",
      schema: "app",
      name: "add",
    ...
    expect(String(fn?.payload["def"])).toContain("SELECT a + b");
    const policy = fb().get({ kind: "policy", schema: "app", table: "users", name: "users_self" });
  ...
  test("comments and ACLs are satellite facts parented to their target", () => {
    ...
    const acl = fb().get({ kind: "acl", target: tableId, grantee: "app_reader_xyz" });
  ...
  test("pg_depend edges arrive at column grain (one granularity, §3.1)", () => {
    ...
    expect(edgeSet.has("view:app.user_emails->column:app.users.email")).toBe(true);
    expect(edgeSet.has("default:app.orders.id->sequence:app.order_seq")).toBe(true);
    // FK constraints reference the target table's columns
    expect(edgeSet.has("constraint:app.orders.orders_user_fk->column:app.users.id")).toBe(true);
  });
  ...
  test("snapshot round-trips a real extraction hash-identically", () => {
    const json = serializeSnapshot(result.factBase, { pgVersion: result.pgVersion });
