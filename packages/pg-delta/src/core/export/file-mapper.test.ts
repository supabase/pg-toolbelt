...
describe("resolveGroupName", () => {
  ...
  test("returns null for cluster-level objects (no schemaName)", () => {
  ...
  describe("auto-detect partitions", () => {
    test("detects partitioned table and returns parent name", () => {
    ...
    test("skips auto-detection when autoPartitions is false", () => {
      const change = tableChange({
        schema: "public",
        name: "events_p20260107",
        isPartition: true,
    ...
    test("chains auto-detect parent name through regex patterns", () => {
    ...
    test("falls back to parent name when no pattern matches", () => {
      const change = tableChange({
        schema: "public",
        name: "events_p20260107",
        isPartition: true,
  ...
  describe("regex patterns", () => {
    ...
    test("matches contains-style regex", () => {
      const change = tableChange({
        schema: "public",
    ...
    test("matches suffix-style regex", () => {
      const change = tableChange({
        schema: "public",
        name: "access_tokens",
      });
    ...
    test("first matching pattern wins (ordering controls priority)", () => {
      const change = tableChange({
        schema: "public",
    ...
    test("skips non-matching patterns and picks the first that matches", () => {
      const change = tableChange({
        schema: "public",
        name: "access_tokens",
      });
    ...
    test("string patterns are compiled as regex", () => {
      const change = tableChange({
        schema: "public",
...
describe("applyGrouping", () => {
  ...
  describe("single-file mode", () => {
    test("replaces filename with group name in same category directory", () => {
  ...
  describe("subdirectory mode", () => {
    test("moves to group-named directory with category as filename", () => {
      const result = applyGrouping(
        baseFilePath,
        "wal_verification_results",
        "subdirectory",
    ...
    test("works for functions category", () => {
...
describe("createFileMapper", () => {
  test("returns getFilePath unchanged when no grouping is provided", () => {
  ...
  test("groups by prefix regex (single-file mode)", () => {
    const mapper = createFileMapper({
      mode: "single-file",
  ...
  test("groups by contains regex (subdirectory mode)", () => {
    const mapper = createFileMapper({
      mode: "subdirectory",
      autoGroupPartitions: false,
  ...
  test("groups by suffix regex (single-file mode)", () => {
    const mapper = createFileMapper({
      mode: "single-file",
      autoGroupPartitions: false,
  ...
  test("does not group unrelated tables", () => {
    const mapper = createFileMapper({
      mode: "single-file",
      autoGroupPartitions: true,
      groupPatterns: [{ pattern: /^project/, name: "project" }],
  ...
  test("first pattern wins when multiple match", () => {
    const mapper = createFileMapper({
      mode: "single-file",
      autoGroupPartitions: false,
      groupPatterns: [
  ...
  test("chains partition auto-detect through regex (kubernetes scenario)", () => {
    const mapper = createFileMapper({
      mode: "single-file",
      autoGroupPartitions: true,
  ...
  test("all pattern types combined end-to-end", () => {
    const mapper = createFileMapper({
      mode: "subdirectory",
      autoGroupPartitions: false,
      groupPatterns: [
  ...
  test("accepts string patterns (compiled as regex)", () => {
    const mapper = createFileMapper({
      mode: "single-file",
      autoGroupPartitions: false,
...
describe("flattenSchema", () => {
  test("collapses tables path to schemas/{schema}/tables.sql", () => {
    const fp: FilePath = {
  ...
  test("collapses functions path to schemas/{schema}/functions.sql", () => {
    const fp: FilePath = {
  ...
  test("leaves schema.sql unchanged", () => {
    const fp: FilePath = {
...
describe("createFileMapper with flatSchemas", () => {
  test("flattens tables in a flat schema to one file", () => {
    const mapper = createFileMapper({
      mode: "single-file",
      flatSchemas: ["partman"],
    });
  ...
  test("does not flatten schemas that are not in the flatSchemas list", () => {
    const mapper = createFileMapper({
      mode: "single-file",
      flatSchemas: ["partman"],
    });
  ...
  test("flat schema takes priority over regex patterns", () => {
    const mapper = createFileMapper({
      mode: "single-file",
      flatSchemas: ["partman"],
  ...
  test("leaves schema.sql unchanged for flat schemas", () => {
    const mapper = createFileMapper({
      mode: "single-file",
      flatSchemas: ["partman"],
    });
  ...
  test("regex patterns still apply to non-flat schemas", () => {
    const mapper = createFileMapper({
      mode: "subdirectory",
      flatSchemas: ["partman"],
      groupPatterns: [{ pattern: /^project/, name: "project" }],
