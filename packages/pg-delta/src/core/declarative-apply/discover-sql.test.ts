...
describe("loadDeclarativeSchema", () => {
  ...
  test("throws when path is a file but not .sql", async () => {
    const dir = path.join(
      tmpdir(),
  ...
  test("loads a single .sql file and returns one entry with relative path", async () => {
    const dir = path.join(
      tmpdir(),
      `pgdelta-discover-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  test("loads directory of .sql files in deterministic order (match pg-topo)", async () => {
    const dir = path.join(
      tmpdir(),
      `pgdelta-discover-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  test("returns empty array when directory has no .sql files", async () => {
    const dir = path.join(
      tmpdir(),
      `pgdelta-discover-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
