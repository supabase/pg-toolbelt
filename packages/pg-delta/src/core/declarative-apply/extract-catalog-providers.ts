...
export const extractCatalogProviders = (db: DatabaseApi) =>
  Effect.gen(function* () {
    ...
    `),
      db.query<ExtensionRow>(`
      SELECT e.extname AS name, n.nspname AS schema
      FROM pg_extension e
      LEFT JOIN pg_namespace n ON n.oid = e.extnamespace
    `),
      db.query<RoleRow>(`SELECT rolname AS name FROM pg_roles`),
      db.query<LanguageRow>(`SELECT lanname AS name FROM pg_language`),
      db.query<CollationRow>(`
      SELECT c.collname AS name, n.nspname AS schema
      FROM pg_collation c
      JOIN pg_namespace n ON n.oid = c.collnamespace
      WHERE n.nspname NOT LIKE 'pg_toast%'
    `),
      db.query<FdwRow>(`SELECT fdwname AS name FROM pg_foreign_data_wrapper`),
      db.query<ServerRow>(`SELECT srvname AS name FROM pg_foreign_server`),
      db.query<EventTriggerRow>(`SELECT evtname AS name FROM pg_event_trigger`),
      db.query<PublicationRow>(`SELECT pubname AS name FROM pg_publication`),
      db.query<SubscriptionRow>(`SELECT subname AS name FROM pg_subscription`),
    ]);
