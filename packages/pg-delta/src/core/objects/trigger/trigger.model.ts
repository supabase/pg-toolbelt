...
export const extractTriggers = (
  ...
  `);
    ...
    const validatedRows = yield* Effect.forEach(triggerRows, (row: unknown) =>
      Schema.decodeUnknownEffect(triggerPropsSchema)(row).pipe(
        Effect.mapError(
          (parseError) =>
            new CatalogExtractionError({
              message: `Schema validation failed in extractTriggers: ${String(parseError)}`,
