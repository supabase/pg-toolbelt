...
order by
  ...
  `);
    ...
    const validatedRows = yield* Effect.forEach(policyRows, (row: unknown) =>
      Schema.decodeUnknownEffect(rlsPolicyPropsSchema)(row).pipe(
        Effect.mapError(
          (parseError) =>
            new CatalogExtractionError({
              message: `Schema validation failed in extractRlsPolicies: ${String(parseError)}`,
