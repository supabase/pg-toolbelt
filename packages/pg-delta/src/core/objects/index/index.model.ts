...
export const extractIndexes = (
  ...
  `);
    ...
    const validatedRows = yield* Effect.forEach(indexRows, (row: unknown) =>
      Schema.decodeUnknownEffect(indexPropsSchema)(row).pipe(
        Effect.mapError(
          (parseError) =>
            new CatalogExtractionError({
              message: `Schema validation failed in extractIndexes: ${String(parseError)}`,
