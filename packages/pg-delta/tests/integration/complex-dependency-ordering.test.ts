...
for (const pgVersion of POSTGRES_VERSIONS) {
  describe(`complex dependency ordering validation (pg${pgVersion})`, () => {
    test(
      "complete e-commerce scenario with all dependency types",
    ...
    test(
      "circular dependency scenario - should fail gracefully",
      withDbIsolated(pgVersion, async (db) => {
        await roundtripFidelityTest({
          mainSession: db.main,
          branchSession: db.branch,
    ...
    test(
      "mixed operation types with complex dependencies",
      withDbIsolated(pgVersion, async (db) => {
        await roundtripFidelityTest({
          mainSession: db.main,
          branchSession: db.branch,
