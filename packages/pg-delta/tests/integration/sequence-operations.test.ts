...
for (const pgVersion of POSTGRES_VERSIONS) {
  describe(`sequence operations (pg${pgVersion})`, () => {
    ...
    test(
      "replace owned sequence and restore the owning column default",
      withDb(pgVersion, async (db) => {
        await roundtripFidelityTest({
          mainSession: db.main,
          branchSession: db.branch,
        ...
        `,
          expectedSqlTerms: [
            "DROP SEQUENCE test_schema.user_id_seq CASCADE",
            "CREATE SEQUENCE test_schema.user_id_seq",
            "ALTER SEQUENCE test_schema.user_id_seq OWNED BY test_schema.users.id",
            "ALTER TABLE test_schema.users ALTER COLUMN id SET DEFAULT nextval('test_schema.user_id_seq'::regclass)",
          ],
        });
      }),
    );
