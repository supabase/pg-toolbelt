...
for (const pgVersion of POSTGRES_VERSIONS) {
  describe(`default privileges dependency ordering (pg${pgVersion})`, () => {
    test(
      "CREATE ROLE must come before ALTER DEFAULT PRIVILEGES FOR ROLE",
    ...
    test(
      "CREATE SCHEMA must come before ALTER DEFAULT PRIVILEGES IN SCHEMA",
      withDbIsolated(pgVersion, async (db) => {
        await roundtripFidelityTest({
          mainSession: db.main,
          branchSession: db.branch,
    ...
    test(
      "CREATE ROLE and CREATE SCHEMA must come before ALTER DEFAULT PRIVILEGES",
      withDbIsolated(pgVersion, async (db) => {
        await roundtripFidelityTest({
          mainSession: db.main,
          branchSession: db.branch,
    ...
    test(
      "constraint spec ensures ALTER DEFAULT PRIVILEGES comes before CREATE TABLE even with dependencies",
      withDbIsolated(pgVersion, async (db) => {
        await roundtripFidelityTest({
          mainSession: db.main,
          branchSession: db.branch,
