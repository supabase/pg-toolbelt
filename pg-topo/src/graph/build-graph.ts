...
import { isKindCompatible, signaturesCompatible } from "../model/object-compat.ts";
import type { Diagnostic, GraphEdgeReason, ObjectRef, StatementNode } from "../model/types.ts";
...
const edgeKey = (fromIndex: number, toIndex: number): string => `${fromIndex}->${toIndex}`;
...
export const buildGraph = (nodes: StatementNode[]): GraphState => {
  ...
  for (const [producerKey, producerIndices] of producersByKey.entries()) {
    ...
    const firstProducer =
      typeof firstProducerIndex === "number" ? nodes[firstProducerIndex] : undefined;
  ...
  for (let consumerIndex = 0; consumerIndex < nodes.length; consumerIndex += 1) {
    ...
    for (const requiredRef of consumer.requires) {
      ...
      if (producerIndices.length === 1) {
        ...
        if (typeof producerIndex === "number" && producerIndex !== consumerIndex) {
      ...
      const compatibleProducerIndices = producerIndicesForRequirement(requiredRef, nodes).filter(
        (index) => index !== consumerIndex,
      );
