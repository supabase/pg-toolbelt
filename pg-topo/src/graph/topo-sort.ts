...
export const compareStatementIndices = (
  ...
  const classDelta =
    statementClassWeight(left.statementClass) - statementClassWeight(right.statementClass);
  if (classDelta !== 0) {
    return classDelta;
  }
...
const insertSorted = (queue: number[], value: number, nodes: StatementNode[]): void => {
...
export const topoSort = (
  ...
  while (queue.length > 0) {
    ...
    neighbors.sort((left, right) => compareStatementIndices(left, right, nodes));
  ...
  const cycleGroups = components
    ...
    .map((component) =>
      [...component].sort((left, right) => compareStatementIndices(left, right, nodes)),
    );
