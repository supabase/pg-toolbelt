...
import { walk as walkSqlAst, type Visitor as SqlAstVisitor } from "@pgsql/traverse";
import {
  DEFAULT_SCHEMA,
  createObjectRef,
...
const typeCastNodeFromExpression = (
  ...
  if (expressionRecord.arg !== undefined && expressionRecord.typeName !== undefined) {
...
const aConstNodeFromExpression = (expressionNode: unknown): Record<string, unknown> | undefined => {
  const expressionRecord = asRecord(expressionNode);
  if (!expressionRecord) {
    return undefined;
  }
...
const inferConstSignaturePart = (constNode: Record<string, unknown>): string => {
  if (constNode.isnull === true) {
    return "unknown";
  }

  const boolNode = asRecord(constNode.boolval);
  if (typeof boolNode?.boolval === "boolean" || typeof constNode.boolval === "boolean") {
    return "bool";
  }
  ...
  if (typeof floatNode?.fval === "number" || typeof floatNode?.fval === "string") {
    return "numeric";
  }
  if (typeof constNode.fval === "number" || typeof constNode.fval === "string") {
    return "numeric";
  }
...
const inferFunctionCallArgumentSignaturePart = (argumentNode: unknown): string => {
...
const inferFunctionCallSignature = (functionCallNode: Record<string, unknown>): string => {
  const args = Array.isArray(functionCallNode.args) ? functionCallNode.args : [];
  const signatureParts = args.map((argNode) => inferFunctionCallArgumentSignaturePart(argNode));
...
const createSqlDependencyVisitor = (
  ...
  RangeVar: (path) => {
    ...
    if (options.qualifiedOnly && typeof rangeVarRecord?.schemaname !== "string") {
      return;
    }
  ...
  FuncCall: (path) => {
    ...
    if (functionRefBase) {
      const functionSignature = inferFunctionCallSignature(functionCallRecord ?? {});
      requires.push(
        createObjectRefFromAst(
          "function",
    ...
    const args = Array.isArray(functionCallRecord?.args) ? functionCallRecord.args : [];
...
export const addExpressionDependencies = (expressionNode: unknown, requires: ObjectRef[]): void => {
  addExpressionDependenciesWithOptions(expressionNode, requires, { qualifiedOnly: false });
};
...
const addQualifiedExpressionDependencies = (
  ...
  addExpressionDependenciesWithOptions(expressionNode, requires, { qualifiedOnly: true });
};
...
const extractRoutineLanguage = (statementNode: Record<string, unknown>): string | null => {
...
const extractRoutineBodySqlBlocks = (statementNode: Record<string, unknown>): string[] => {
  const blocks: string[] = [];
  for (const defElem of functionOptionElements(statementNode)) {
...
const addSqlRoutineBodyDependencies = (bodyBlock: string, requires: ObjectRef[]): void => {
...
const addPlpgsqlRoutineBodyDependencies = (bodyBlock: string, requires: ObjectRef[]): void => {
  ...
  const parsedFunction =
    Array.isArray(parsedFunctionScript?.functions) && parsedFunctionScript.functions.length > 0
      ? parsedFunctionScript.functions[0]
      : undefined;
  const hydratedPlpgsqlAst = asRecord(asRecord(parsedFunction)?.plpgsql)?.hydrated;
