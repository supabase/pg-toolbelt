import { DEFAULT_SCHEMA, createObjectRefFromAst } from "../model/object-ref.ts";
...
export const objectKindFromObjType = (objType: unknown): ObjectRef["kind"] | null => {
  switch (objType) {
...
export const objectFromNameParts = (
  ...
  return createObjectRefFromAst(kind, parts.at(-1) ?? "", parts.at(-2) ?? fallbackSchema);
};

export const typeFromTypeNameNode = (typeNameNode: unknown): ObjectRef | null => {
...
export const relationFromRangeVarNode = (
  ...
  const relname = typeof rangeVar.relname === "string" ? rangeVar.relname : undefined;
  if (!relname) {
    return null;
  }

  const schema = typeof rangeVar.schemaname === "string" ? rangeVar.schemaname : DEFAULT_SCHEMA;
...
export const addForeignConstraintDependencies = (
  ...
  if (foreignTable) {
    ...
    const referencedKey = keyRefForTableColumns(foreignTable, constraintKeyColumns(constraint));
    if (referencedKey) {
...
export const addSchemaDependencyIfNeeded = (schemaName: unknown, requires: ObjectRef[]): void => {
...
export const parseNamedObjectRef = (
  ...
  const rangeVarRef = relationFromRangeVarNode(asRecord(nodeRecord.RangeVar), kind);
  if (rangeVarRef) {
    return rangeVarRef;
  }
  ...
  if (objectWithArgs) {
    ...
    const args = Array.isArray(objectWithArgs.objargs) ? objectWithArgs.objargs : [];
    if (args.length === 0) {
      return baseRef;
    }
    ...
    const signatureParts = args.map((argNode) => {
      ...
      return typeRef.schema ? `${typeRef.schema}.${typeRef.name}` : typeRef.name;
    });
