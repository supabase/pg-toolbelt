import { isKindCompatible, signaturesCompatible } from "../../../src/model/object-compat";
import { normalizeSignature, splitQualifiedName } from "../../../src/model/object-ref";
...
const inferMissingTypeKindFromStatement = (
  ...
  matches.sort((left, right) => {
    const leftPriority = relationLikeKindPriorityForTypeLookup.indexOf(left.kind);
    const rightPriority = relationLikeKindPriorityForTypeLookup.indexOf(right.kind);
    return leftPriority - rightPriority;
  });
...
export const inferMissingObjectRef = (
  ...
  if (code === "42703") {
    ...
    const inferredKind =
      inferMissingRelationKindFromStatement(statementNode, schema, name) ?? "table";
    return {
      kind: inferredKind,
      schema,
      name,
  ...
  if (code === "42704") {
    ...
    if (typeMatch) {
      ...
      const { schema, name } = splitQualifiedName(objectName, "ast");
      const inferredKind = inferMissingTypeKindFromStatement(statementNode, schema, name) ?? "type";
      return {
        kind: inferredKind,
        schema,
        name,
...
export const findProducerCandidates = (missingRef: ObjectRef, nodes: StatementNode[]): number[] => {
  const candidates: number[] = [];
  for (let producerIndex = 0; producerIndex < nodes.length; producerIndex += 1) {
