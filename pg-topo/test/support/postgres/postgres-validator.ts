...
import { findProducerCandidates, inferMissingObjectRef } from "./postgres-missing-ref";
import {
    ...
    async (db) => {
      ...
      for (const statementIndex of topoResult.orderedIndices) {
        ...
        try {
          ...
          const producerCandidates = findProducerCandidates(missingRef, nodes).filter(
            (producerIndex) => producerIndex !== statementIndex,
          );
          ...
          if (typeof producerIndex !== "number") {
            diagnostics.push({
              code: "RUNTIME_EXECUTION_ERROR",
              message: "Internal error while selecting an internal producer candidate.",
              statementId: statementNode.id,
              details: { sqlstate: dbError.code },
            });
            aborted = true;
