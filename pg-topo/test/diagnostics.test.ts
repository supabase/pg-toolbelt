...
describe("diagnostics", () => {
  test("reports duplicate producers with candidate details", async () => {
    const root = await createSqlFixture({
      ...
      "02_users_v2.sql": "create table app.users(id int primary key, email text not null);",
    ...
    expect(`${ambiguousDependency?.details?.candidateObjectKeys ?? ""}`).toContain(
      "table:app:users",
    );
  });
  ...
  test("includes candidate producers for unresolved dependencies", async () => {
    const root = await createSqlFixture({
      ...
      "02_view.sql": "create view public.account_ids as select id from public.accounts;",
    });
  ...
  test("cycle diagnostics include statement participants", async () => {
    ...
    expect(`${cycleDiagnostic?.details?.cycleStatements ?? ""}`).toContain("00_v1.sql");
    expect(`${cycleDiagnostic?.details?.cycleStatements ?? ""}`).toContain("01_v2.sql");
  });
});
