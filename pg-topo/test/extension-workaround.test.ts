...
describe("postgres runtime validation", () => {
  test("keeps extension object dependencies unresolved in static analysis", async () => {
    const root = await createSqlFixture({
      ...
      "02_extension.sql": 'create extension if not exists "uuid-ossp" with schema extensions;',
    });
  ...
  test("validator executes installed extensions on postgres-alpine", async () => {
    const root = await createSqlFixture({
      ...
      "01_extension.sql": 'create extension if not exists "uuid-ossp" with schema extensions;',
      "02_table.sql":
        "create table public.demo(id uuid default extensions.uuid_generate_v4() primary key);",
    });
  ...
  test("validator assumes external dependencies when producer is absent and no bootstrap migration is provided", async () => {
    const root = await createSqlFixture({
      "00_view.sql": "create view public.external_users as select * from external.users;",
    });
  ...
  test("sorts comments RLS/FK/index/trigger/policy statements and executes with bootstrap dependencies", async () => {
    const root = await createSqlFixture({
      ...
      "02_index_post.sql": "create index idx_comments_post_id on public.comments (post_id);",
      "03_trigger.sql":
      ...
      "06_enable_rls.sql": "alter table public.comments enable row level security;",
      ...
      "08_pk.sql": "alter table public.comments add constraint comments_pkey primary key (id);",
      "09_fk_parent.sql":
      ...
      "10_index_author.sql": "create index idx_comments_author_id on public.comments (author_id);",
    });
