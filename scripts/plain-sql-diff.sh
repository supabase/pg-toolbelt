...
VERIFY_OUTPUT=$(pnpm pgdelta plan --source "$SHADOW_URL" --target "$TARGET_URL" --filter "$FILTER_DSL" 2>&1) || true
if echo "$VERIFY_OUTPUT" | grep -q "No changes detected."; then
  ...
  pnpm pgdelta plan --source "$SHADOW_URL" --target "$TARGET_URL" --filter "$FILTER_DSL" --format sql
  echo ""
